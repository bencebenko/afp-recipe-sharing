...
flowchart TD
    ...
    %% === ALAPANYAG attribútumok ===
    aid(["<u>id</u>"])
    aname(["neve"])
    akal(["kaloria"])
    afeherje(["feherje"])
    aszenhidrat(["szenhidrat"])
    azsir(["zsir"])
    amertek(["mertekegyseg"])

    ALAPANYAG --- aid
    ALAPANYAG --- aname
    ALAPANYAG --- akal
    ALAPANYAG --- afeherje
    ALAPANYAG --- aszenhidrat
    ALAPANYAG --- azsir
    ALAPANYAG --- amertek

    %% === HOZZAVALOK attribútumok ===
    hid(["<u>id</u>"])
    hrecept(["recept_id"])
    halap(["alapanyag_id"])
    hmenny(["mennyiseg"])
    ...
    %% === KAPCSOLATOK BEKÖTÉSE ===
    USER --- R_IRJA --- RECEPT
    USER --- R_IR --- KOMMENT
    USER --- R_KEDV --- KEDVENCEK

    RECEPT --- R_TART --- HOZZAVALOK
    RECEPT --- R_KAP --- KOMMENT
    RECEPT --- R_SZEREPEL --- KEDVENCEK
    RECEPT --- R_BESOR --- KATEGORIA

    HOZZAVALOK --- R_HASZNAL --- ALAPANYAG
```
...
erDiagram
    ...
    USER ||--o{ RECEPT : "írja"
    USER ||--o{ KOMMENT : "ír"
    USER ||--o{ KEDVENCEK : "kedvencel"

    RECEPT ||--o{ HOZZAVALOK : "tartalmaz"
    RECEPT ||--o{ KOMMENT : "kap"
    RECEPT ||--o{ KEDVENCEK : "szerepel"
    RECEPT o|--|| KATEGORIA : "besorolva"
    HOZZAVALOK }o--|| ALAPANYAG : "használ"

    %% === STÍLUSOK ===
    style USER fill:#1E90FF,stroke:#87CEFA,stroke-width:1px,color:#ddd
    style RECEPT fill:#2E8B57,stroke:#98FB98,stroke-width:1px,color:#ddd
    style HOZZAVALOK fill:#3CB371,stroke:#90EE90,stroke-width:1px,color:#ddd
    style ALAPANYAG fill:#DAA520,stroke:#FFD700,stroke-width:1px,color:#ddd
    style KATEGORIA fill:#8B4513,stroke:#CD853F,stroke-width:1px,color:#ddd
    style KOMMENT fill:#B22222,stroke:#FF7F7F,stroke-width:1px,color:#ddd
    style KEDVENCEK fill:#C71585,stroke:#FFB6C1,stroke-width:1px,color:#ddd
