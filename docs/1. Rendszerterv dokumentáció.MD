...
- **Deák Dániel**
    ...
    - fő komponensek implementálása (Home, Recipe, Account, AddRecipe, EditRecipe, Register dialógus)
    - backend API-k integrálása
    - kézi, exploratív tesztelés támogatása
- **Benkő Bence**
    - UI/UX tervezés Figma-ban
    - manuális teszt készítése a frontendre
    - bugfixek
    - fő komponens implementálása (Profil)
    - felhasználói élmény finomhangolása
- **Csajbók-Reményi Korinna**
    - frontend funkciók, mellékkomponensek implementálása (CategoryFilter, Searchbar, Responsive Navbar)
    - kézi, exploratív tesztelés támogatása
...
- Dokumentáció (funkcionális specifikáció, rendszerterv): teljes fejlesztői csapat
- BDD tesztek: Pataricza György
- Unit és MockMvc alapú controller tesztek: Németh Dávid
- Frontend tesztek: Benkő Bence, Csajbók-Reményi Korinna, Deák Dániel
- Rendszer- és integrációs tesztek: teljes csapat
...
flowchart LR
    ...
    subgraph "Recept módosítása folyamat"
        direction LR
        A((Start)):::startstop --> B["Bejelentkezés"]:::action
        B --> C["Edit recipe űrlap megnyitása"]:::action
        C --> D["Adatok kitöltése"]:::action
        D --> E{"Adatok érvényesek?"}:::decision
        E -->|Nem| D:::error
        E -->|Igen| F["Recept mentése (POST /recipe)"]:::action
...
![PlantUML model](http://www.plantuml.com/plantuml/png/5Sqz3i8m343XdLF00HhlJ1M8WxYEsqGaJkGVSNtQvRtnMyrHFNgxFQfGYyoXxLxSfpq157_fg5vY2sEb8SxYYKQ7ZONup7S0xdDHfZfviO9QvG3btQ0mPeEELI0FCdYzFsbUqigIp_y1)
...
Mobilnézetben egységesen drawer menüben jelennek meg a menüpontok bejelentkezettségnek megfelelően.
...
- **Recipe details:** kép, leírás, hozzávalók, előkészítési idő, elkészítési idő, adag, hozzávalók, elkészítés lépései, tápértékek
...
flowchart LR
    ...
    uname(["name"])
    uusername(["userName"])
    upass(["passwordEncoded"])
    uemail(["email"])
    upic(["profileImageRef"])

    RECIPE_USER --- uid
    RECIPE_USER --- uname
    RECIPE_USER --- uusername
    RECIPE_USER --- upass
    RECIPE_USER --- uemail
    RECIPE_USER --- upic
    ...
    R_WRITES{"írja"}
    R_CLASS{"besorolva"}
    R_CONTAINS{"tartalmaz"}
    R_USES{"használ"}

    RECIPE_USER --- R_WRITES --- RECIPE
    RECIPE --- R_CONTAINS --- INGREDIENT
    RECIPE --- R_CLASS --- CATEGORY
    INGREDIENT --- R_USES --- MATERIAL
```
---
#### Adatbázis séma:
```mermaid
...
- **Oldalak:** `Home`, `Recipe`, `Profil`, `Account`
- **Komponensek:** `NavMenu`, `AccountNavMenu`, `CategoryFilter`, `AddRecipeDialog`, `EditRecipeDialog`, `RegisterRecipeDialog`
...
- **Frontend tesztek:** Benkő Bence, Csajbók-Reményi Korinna, Deák Dániel
- **Manuális rendszer- és integrációs tesztek:** Teljes csapat
- **Explorációs manuális tesztelés:** Teljes csapat
